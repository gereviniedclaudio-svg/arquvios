...
try {
    ...
    // ========== CONSULTA STATUS NA MEDUSA PAY ==========
    $ch = curl_init($MEDUSA_BASE . '/api/pagamentos/' . rawurlencode($id));
    ...
    if ($httpCode >= 200 && $httpCode < 300 && is_array($tx)) {
        ...
        $isPaid = in_array($statusRaw, ['aprovado', 'pago', 'paid', 'approved', 'completed'], true);
        $status = $isPaid ? 'paid' : 'pending';
    } elseif ($pedido) {
        // API indisponível: usa o último status conhecido no banco
        error_log("[Verificar Medusa] ⚠️ API indisponível (HTTP $httpCode). Usando status do banco.");
    ...
    if ($isPaid && $pedido && $pedido['status'] !== 'paid') {
        ...
        if ($update->rowCount() > 0) {
            error_log("[Verificar Medusa] ✅ Pagamento aprovado: $id. Notificando Utmify...");
            ...
            error_log("[Verificar Medusa] 📡 Utmify paid (HTTP $utmifyHttpCode): " . $utmifyResponse);
        }
    }

} catch (Exception $e) {
    error_log("[Verificar Medusa] ❌ Erro: " . $e->getMessage());
    echo json_encode([
        'success' => false,
        'status'  => 'error',
